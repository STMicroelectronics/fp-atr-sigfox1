...
#if (osCMSIS < 0x20000U)
  osThreadId teseoConsumerThreadId = NULL;
#else
  osThreadId_t teseoConsumerThreadId = NULL;
#endif /* osCMSIS */
#endif /* USE_GNSS */
...
#ifdef USE_GNSS
#if (osCMSIS < 0x20000U)
  osThreadDef(Thread2, TeseoConsumerTask, osPriorityNormal, 0, CONSUMER_STACK_SIZE);
  teseoConsumerThreadId = osThreadCreate(osThread(Thread2), NULL);
#else
  task_attributes.name = "teseoConsumerTask";
  task_attributes.stack_size = CONSUMER_STACK_SIZE;
  teseoConsumerThreadId = osThreadNew(TeseoConsumerTask, NULL, (const osThreadAttr_t *)&task_attributes);
#endif /* osCMSIS */
#endif /* USE_GNSS */

#if (osCMSIS < 0x20000U)
  osThreadDef(Thread1, MX_FP_ATR_SIGFOX1_Process, osPriorityNormal, 0, MAIN_THREAD_STACK_SIZE);
  MainThreadId = osThreadCreate (osThread(Thread1), NULL);
#else
  task_attributes.name = "MainThread";
  task_attributes.stack_size = MAIN_THREAD_STACK_SIZE;
  MainThreadId = osThreadNew(MX_FP_ATR_SIGFOX1_Process, NULL, (const osThreadAttr_t *)&task_attributes);
#endif /* osCMSIS */
}
...
#endif /* osCMSIS */
{
  ...
  while(1)
  {
...
#ifdef USE_GNSS
    osThreadSuspend(teseoConsumerThreadId);
#endif
    ...
    setGpioRestore();
#ifdef USE_GNSS
    osThreadResume(teseoConsumerThreadId);
#endif
